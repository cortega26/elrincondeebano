(()=>{var S=(t,r)=>()=>(t&&(r=t(t=0)),r);var Tt=(t,r)=>()=>(r||t((r={exports:{}}).exports,r),r.exports);function tt(){try{let t=document.createElement("style");window&&window.__CSP_NONCE__&&t.setAttribute("nonce",window.__CSP_NONCE__),t.textContent=`
    ...
    `,d.querySelector(".primary-action").addEventListener("click",()=>{m(),d.remove()}),d.querySelector(".secondary-action").addEventListener("click",()=>{d.remove()}),d}function J(t){let r=document.querySelector(".notification-toast");r&&r.remove(),document.body.appendChild(t),setTimeout(()=>{document.body.contains(t)&&t.remove()},300*1e3)}var K,Ut,Wt,V,Qt,Q,Kt,u,v,yt,ht,vt=S(()=>{pt();K={path:"/service-worker.js",scope:"/",updateCheckInterval:300*1e3};typeof navigator<"u"&&"serviceWorker"in navigator&&gt();Ut=(t,r=100)=>{let s=new Map;return(...m)=>{let d=JSON.stringify(m);if(s.has(d))return s.get(d);let f=t(...m);if(s.size>=r){let g=s.keys().next().value;s.delete(g)}return s.set(d,f),f}},Wt=(t,r)=>{let s;return(...m)=>{clearTimeout(s),s=setTimeout(()=>t(...m),r)}},V=t=>{if(!t)return"";try{return String(t).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9]/g,"").toLowerCase()}catch{return String(t).toLowerCase()}},Qt=t=>{let r=`${t.name}-${t.category}`.toLowerCase(),s=0;for(let m=0;m<r.length;m++){let d=r.charCodeAt(m);s=(s<<5)-s+d,s=s&s}return`pid-${Math.abs(s)}`},Q=t=>{let r=document.createElement("div");return r.textContent=t,r.innerHTML},Kt=async()=>{try{let t=localStorage.getItem("productDataVersion"),r=t?`/data/product_data.json?v=${encodeURIComponent(t)}`:"/data/product_data.json",s=await fetch(r,{cache:"no-store",headers:{Accept:"application/json"}});if(!s.ok)throw new Error(`HTTP error. Status: ${s.status}`);return(await s.json()).products.map(d=>({...d,id:Qt(d),name:Q(d.name),description:Q(d.description),category:Q(d.category),categoryKey:V(d.category)}))}catch(t){throw console.error("Error al obtener productos:",t),v(`Error al cargar los productos. Por favor, verifique su conexi\xF3n a internet e int\xE9ntelo de nuevo. (Error: ${t.message})`),t}},u=(t,r={},s=[])=>{let m=document.createElement(t);return Object.entries(r).forEach(([d,f])=>{d==="text"?m.textContent=f:m.setAttribute(d,f)}),s.forEach(d=>{typeof d=="string"?m.appendChild(document.createTextNode(d)):m.appendChild(d)}),m},v=t=>{let r=u("div",{class:"error-message",role:"alert"},[u("p",{},[t]),u("button",{class:"retry-button"},["Intentar nuevamente"])]),s=document.getElementById("product-container");s?(s.innerHTML="",s.appendChild(r),r.querySelector(".retry-button").addEventListener("click",ht)):console.error("Contenedor de productos no encontrado")},yt=[];try{yt=JSON.parse(globalThis.localStorage?.getItem("cart"))||[]}catch{yt=[]}typeof window<"u"&&(window.__APP_READY__=!1,window.addEventListener("error",t=>{let r=t.target||t.srcElement,s=!!(r&&(r.tagName==="IMG"||r.tagName==="SCRIPT"||r.tagName==="LINK")),m=!!t.error;if(s||!m){console.warn("Ignored non-fatal error:",{tag:r&&r.tagName,src:r&&(r.src||r.href||r.currentSrc)});return}if(!window.__APP_READY__){console.error("Runtime error before app ready:",t.error);return}console.error("Unhandled JS error:",t.error),v("Ocurri\xF3 un error inesperado. Por favor, recarga la p\xE1gina.")}),window.addEventListener("securitypolicyviolation",t=>{console.warn("CSP violation (logged only):",{blockedURI:t.blockedURI,violatedDirective:t.violatedDirective,sourceFile:t.sourceFile})}));ht=async()=>{if(console.log("Initializing app..."),typeof DOMPurify>"u")try{await new Promise((e,o)=>{let a=document.createElement("script");a.src="https://cdnjs.cloudflare.com/ajax/libs/dompurify/2.3.8/purify.min.js",a.integrity="sha384-AsiVBlzbaNOq8OOKcXm2ZVjjKJwiQ9UmzLwfetDjC74OMQdkb6vBHH5QRJH3x1SE",a.crossOrigin="anonymous",a.onload=e,a.onerror=o,document.head.appendChild(a)})}catch{console.warn("DOMPurify CDN blocked; using noop sanitizer"),window.DOMPurify={sanitize:o=>o}}let t=document.getElementById("navbar-container"),r=document.getElementById("footer-container"),s=document.getElementById("product-container"),m=document.getElementById("sort-options"),d=document.getElementById("filter-keyword"),f=()=>{let e=document.getElementById("filter-discount");if(e)return e;let o=document.querySelector('section[aria-label*="filtrado"], section[aria-label*="Opciones de filtrado"]'),a=o?o.querySelector(".row"):null;if(!a)return null;let i=u("div",{class:"col-12 mt-2"}),n=u("div",{class:"form-check form-switch"}),c=u("input",{class:"form-check-input",type:"checkbox",id:"filter-discount","aria-label":"Mostrar solo productos con descuento"}),l=u("label",{class:"form-check-label",for:"filter-discount"},["Solo productos con descuento"]);return n.appendChild(c),n.appendChild(l),i.appendChild(n),a.appendChild(i),c},g=[],y=JSON.parse(localStorage.getItem("cart"))||[],j=()=>{let e=document.getElementById("offline-indicator");if(e){let o=navigator.onLine;e.classList.toggle("is-hidden",o),e.style.display=o?"none":"block"}navigator.onLine||console.log("App is offline. Using cached data if available.")},Y=async(e,o)=>{if(!e){console.warn(`Contenedor no encontrado para el componente: ${o}`);return}try{let a=await fetch(o);if(!a.ok)throw new Error(`HTTP error! status: ${a.status}`);let i=await a.text(),n=DOMPurify.sanitize(i,{USE_PROFILES:{html:!0},ALLOWED_TAGS:["div","span","p","h1","h2","h3","h4","h5","h6","ul","ol","li","a","img","br","strong","em","button","nav","footer","header","main","section"],ALLOWED_ATTR:["href","src","alt","class","id","style","aria-label","role","type","data-bs-toggle","data-bs-target","aria-controls","aria-expanded"]});e.innerHTML="";let l=new DOMParser().parseFromString(n,"text/html");Array.from(l.body.children).forEach(p=>{e.appendChild(p.cloneNode(!0))})}catch(a){throw console.error("Error al cargar componente:",{component:o,message:a.message}),a}};function bt(){let e=document.getElementById("current-year");e&&(e.textContent=new Date().getFullYear())}let Ct=async()=>{try{await Promise.all([Y(t,"/pages/navbar.html"),Y(r,"/pages/footer.html")]),console.log("Components loaded successfully"),bt()}catch(e){console.error("Error al cargar componentes:",e),v("Error al cargar los componentes de la p\xE1gina. Por favor, actualice la p\xE1gina o verifique su conexi\xF3n a internet.")}},Et=(e,o,a="CLP")=>{let i=new Intl.NumberFormat("es-CL",{style:"currency",currency:a,minimumFractionDigits:0}),n=i.format(e);if(o){let c=e-o,l=i.format(c);return u("div",{class:"precio-container"},[u("span",{class:"precio-descuento","aria-label":"Precio con descuento"},[l]),u("span",{class:"precio-original","aria-label":"Precio original"},[u("span",{class:"tachado"},[n])])])}return u("div",{class:"precio-container"},[u("span",{class:"precio","aria-label":"Precio"},[n])])},St=e=>{let o=u("div",{class:"quantity-control"}),a=u("button",{class:"quantity-btn","aria-label":"Decrease quantity"},["-"]),i=u("button",{class:"quantity-btn","aria-label":"Increase quantity"},["+"]),n=u("input",{type:"number",class:"quantity-input",value:Math.max(R(e.id),1),min:"1",max:"50","aria-label":"Quantity","data-id":e.id});return a.addEventListener("click",()=>A(e,-1)),i.addEventListener("click",()=>A(e,1)),n.addEventListener("change",c=>{let l=parseInt(c.target.value,10),p=R(e.id);A(e,l-p)}),o.appendChild(a),o.appendChild(n),o.appendChild(i),o},R=e=>{let o=y.find(a=>a.id===e);return o?o.quantity:0},Lt=e=>{let o=document.createDocumentFragment();e.forEach(i=>{let{id:n,name:c,description:l,image_path:p,price:L,discount:q,stock:U}=i,z=u("div",{class:`producto col-12 col-sm-6 col-md-4 col-lg-3 mb-4 ${U?"":"agotado"}`,"aria-label":`Product: ${c}`}),k=u("div",{class:"card"});if(q&&Number(q)>0){let I=Math.round(Number(q)/Number(L)*100),B=u("span",{class:"discount-badge badge bg-danger","aria-label":"Producto en oferta"},[`-${isFinite(I)?I:0}%`]);k.appendChild(B)}let _=`/${p.replace(/^\//,"")}`,$=u("img",{src:O(_,{...T,width:400}),srcset:[`${O(_,{...T,width:200})} 200w`,`${O(_,{...T,width:400})} 400w`,`${O(_,{...T,width:800})} 800w`].join(", "),sizes:"(max-width: 640px) 200px, 400px",alt:c,class:"card-img-top product-thumb",loading:"lazy",decoding:"async",width:"400",height:"400"});k.appendChild($);let w=u("div",{class:"card-body"});w.appendChild(u("h3",{class:"card-title"},[c])),w.appendChild(u("p",{class:"card-text"},[l])),w.appendChild(Et(L,q));let D=u("div",{class:"action-area","data-pid":n}),C=u("button",{class:"btn btn-primary add-to-cart-btn mt-2","data-id":n,"aria-label":`Add ${c} to cart`},["Agregar al carrito"]),E=St(i);E.style.display="none",C.addEventListener("click",()=>{Z(i,1),M(C,E,!0)}),D.appendChild(C),D.appendChild(E),w.appendChild(D);let b=y.find(I=>I.id===n),h=b?b.quantity:0,W=E.querySelector(".quantity-input");h>0?(W&&(W.value=h),C.style.display="none",E.style.display="flex"):(C.style.display="flex",E.style.display="none"),k.appendChild(w),z.appendChild(k),o.appendChild(z)});let a=document.getElementById("product-container");a.innerHTML="",a.appendChild(o),It()},It=()=>{let e=new IntersectionObserver((o,a)=>{o.forEach(i=>{if(i.isIntersecting){let n=i.target;n.src=n.dataset.src,n.dataset.srcset&&(n.srcset=n.dataset.srcset),n.dataset.sizes&&(n.sizes=n.dataset.sizes),n.classList.remove("lazyload"),a.unobserve(n)}})},{rootMargin:"100px"});document.querySelectorAll("img.lazyload").forEach(o=>e.observe(o))};function Pt(e,o){if(!e||!o||e.length<3)return!1;let a=c=>c.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").trim(),i=a(e),n=a(o);return n.includes(i)?!0:i.length>=4?qt(i,n):!1}function qt(e,o){return o.split(/\s+/).some(i=>{if(Math.abs(i.length-e.length)>1)return!1;let n=0,c=Math.max(i.length,e.length);if(Math.min(i.length,e.length)<4)return!1;if(i.length===e.length){for(let p=0;p<i.length;p++)if(i[p]!==e[p]&&n++,n>1)return!1;return n===1}else return kt(e,i)})}function kt(e,o){let a=e.length>o.length?e:o,i=e.length>o.length?o:e;if(a.length-i.length!==1)return!1;let n=0,c=0,l=0;for(;c<i.length&&l<a.length;)if(i[c]===a[l])c++,l++;else{if(n++,n>1)return!1;l++}return!0}let _t=(e,o,a,i=!1)=>{let n=o.trim();return e.filter(c=>!c.stock||i&&!(c.discount&&Number(c.discount)>0)?!1:!n||c.name.toLowerCase().includes(n.toLowerCase())||c.description.toLowerCase().includes(n.toLowerCase())?!0:n.length>=4?Pt(n,c.name):!1).sort((c,l)=>xt(c,l,a))},xt=(e,o,a)=>{if(!a||a==="original")return e.originalIndex-o.originalIndex;let[i,n]=a.split("-"),c=i==="price"?e.price-(e.discount||0):e.name.toLowerCase(),l=i==="price"?o.price-(o.discount||0):o.name.toLowerCase();return n==="asc"?c<l?-1:c>l?1:0:l<c?-1:l>c?1:0},At=Ut(_t),F=()=>{try{let e=m.value||"original",o=d.value.trim(),a=document.getElementById("filter-discount")?.checked||!1,i=At(g,o,e,a);Lt(i)}catch(e){console.error("Error al actualizar visualizaci\xF3n de productos:",e),v("Error al actualizar la visualizaci\xF3n de productos. Por favor, intenta m\xE1s tarde.")}},H=Wt(F,300),x=()=>{let e=document.getElementById("cart-count"),o=y.reduce((a,i)=>a+i.quantity,0);e.textContent=o,e.setAttribute("aria-label",`${o} items in cart`)},Z=(e,o)=>{try{let a=y.find(n=>n.id===e.id);a?a.quantity=Math.min(a.quantity+o,50):y.push({id:e.id,name:e.name,description:e.description,price:e.price,discount:e.discount,image_path:e.image_path,quantity:Math.min(o,50),category:e.category,stock:e.stock}),N(),x(),P();let i=document.querySelector(`[data-id="${e.id}"].quantity-input`);i&&(i.value=Math.max(R(e.id),1))}catch(a){console.error("Error al agregar al carrito:",a),v("Error al agregar el art\xEDculo al carrito. Por favor, intenta nuevamente.")}},X=e=>{try{y=y.filter(a=>a.id!==e),N(),x(),P();let o=document.querySelector(`.action-area[data-pid="${e}"]`);if(o){let a=o.querySelector(".add-to-cart-btn"),i=o.querySelector(".quantity-control");M(a,i,!1)}}catch(o){console.error("Error al eliminar del carrito:",o),v("Error al eliminar el art\xEDculo del carrito. Por favor, intenta nuevamente.")}},A=(e,o)=>{try{let a=y.find(p=>p.id===e.id),i=a?a.quantity+o:1,n=document.querySelector(`.action-area[data-pid="${e.id}"]`),c=n?.querySelector(".add-to-cart-btn"),l=n?.querySelector(".quantity-control");if(i<=0)X(e.id),M(c,l,!1);else if(i<=50){a?a.quantity=i:(Z(e,1),M(c,l,!0)),N(),x(),P();let p=document.querySelector(`[data-id="${e.id}"].quantity-input`);p&&(p.value=i,p.classList.add("quantity-changed"),setTimeout(()=>p.classList.remove("quantity-changed"),300))}}catch(a){console.error("Error al actualizar cantidad:",a),v("Error al actualizar la cantidad. Por favor, intenta nuevamente.")}},$t=()=>{try{y=[],N(),x(),P(),F()}catch(e){console.error("Error al vaciar el carrito:",e),v("Error al vaciar el carrito. Por favor, int\xE9ntelo de nuevo.")}},M=(e,o,a)=>{!e||!o||(a?(e.style.display="none",o.style.display="flex"):(o.style.display="none",e.style.display="flex"))},N=()=>{try{localStorage.setItem("cart",JSON.stringify(y))}catch(e){console.error("Error al guardar el carrito:",e),v("Error al guardar el carrito. Tus cambios podr\xEDan no persistir.")}},P=()=>{let e=document.getElementById("cart-items"),o=document.getElementById("cart-total");e.innerHTML="";let a=0;y.forEach(n=>{let c=n.price-(n.discount||0),l=u("div",{class:"cart-item mb-3 d-flex align-items-start","aria-label":`Cart item: ${n.name}`}),p=u("div",{class:"cart-item-content flex-grow-1"});p.appendChild(u("div",{class:"fw-bold mb-1"},[n.name]));let L=u("div",{class:"mb-2"}),q=u("button",{class:"btn btn-sm btn-secondary decrease-quantity","data-id":n.id,"aria-label":`Disminuir cantidad de ${n.name}`},["-"]),U=u("button",{class:"btn btn-sm btn-secondary increase-quantity","data-id":n.id,"aria-label":`Aumentar cantidad de ${n.name}`},["+"]),z=u("span",{class:"mx-2 item-quantity","aria-label":`Cantidad de ${n.name}`},[n.quantity.toString()]);L.appendChild(q),L.appendChild(z),L.appendChild(U),p.appendChild(L),p.appendChild(u("div",{class:"text-muted small"},[`Precio: $${c.toLocaleString("es-CL")}`])),p.appendChild(u("div",{class:"fw-bold"},[`Subtotal: $${(c*n.quantity).toLocaleString("es-CL")}`]));let k=u("button",{class:"btn btn-sm btn-danger remove-item mt-2","data-id":n.id,"aria-label":`Eliminar ${n.name} del carrito`},["Eliminar"]);p.appendChild(k);let _=window.location.pathname.includes("/pages/"),$;_?$=`../${n.image_path.replace(/^\//,"")}`:$=n.image_path;let w=u("div",{class:"cart-item-thumb ms-3 flex-shrink-0"}),C={src:n.thumbnail_path||$,alt:n.name,class:"cart-item-thumb-img",loading:"lazy",decoding:"async",width:"100",height:"100"};if(Array.isArray(n.thumbnail_variants)){let b=n.thumbnail_variants.filter(h=>h&&h.url&&h.width).map(h=>`${h.url} ${h.width}w`);b.length&&(C.srcset=b.join(", "),C.sizes="100px")}let E=u("img",C);if(w.appendChild(E),l.appendChild(p),l.appendChild(w),e.appendChild(l),window.__DEBUG_CART__)try{let b=window.getComputedStyle(l),h=window.getComputedStyle(w),W=window.getComputedStyle(p),I=w.getBoundingClientRect(),B=p.getBoundingClientRect();console.debug("[cart] item",{name:n.name,order:{firstIsContent:l.firstChild===p,lastIsThumb:l.lastChild===w},container:{display:b.display,flexDirection:b.flexDirection,flexWrap:b.flexWrap},thumb:{width:h.width,height:h.height,rect:{w:I.width,h:I.height}},content:{rect:{w:B.width,h:B.height}}})}catch{}a+=c*n.quantity}),o.textContent=`Total: $${a.toLocaleString("es-CL")}`,o.setAttribute("aria-label",`Total: $${a.toLocaleString("es-CL")}`);let i=document.getElementById("payment-credit-container");if(i)if(a>=3e4)i.classList.remove("d-none");else{i.classList.add("d-none");let n=i.querySelector("input");n&&(n.checked=!1)}},Ot=()=>{let e=document.querySelector('input[name="paymentMethod"]:checked');if(!e){alert("Por favor seleccione un m\xE9todo de pago");return}let o=`Mi pedido:
