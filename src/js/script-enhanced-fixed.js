  ...
  loadComponent: async (container, filename) => {
    ...
    try {
      ...
      if (ldJsonBlocks.length) {
        const nonce =
          typeof window !== 'undefined' && window.__CSP_NONCE__ ? window.__CSP_NONCE__ : null;
    ...
    if (typeof DOMPurify === 'undefined') {
      try {
        await new Promise((resolve, reject) => {
          ...
          script.integrity =
            'sha384-AsiVBlzbaNOq8OOKcXm2ZVjjKJwiQ9UmzLwfetDjC74OMQdkb6vBHH5QRJH3x1SE';
          script.crossOrigin = 'anonymous';
          try {
            if (window && window.__CSP_NONCE__) script.setAttribute('nonce', window.__CSP_NONCE__);
          } catch {}
