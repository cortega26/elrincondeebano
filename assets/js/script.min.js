"use strict";(()=>{var ue=(n,i)=>()=>(i||n((i={exports:{}}).exports,i),i.exports);var Le=ue((Se,W)=>{var j={path:"/service-worker.js",scope:"/",updateCheckInterval:3e5};function Y(){if(!("serviceWorker"in navigator)){console.warn("Service workers are not supported in this browser");return}window.addEventListener("load",async()=>{try{pe()}catch(n){console.error("Service Worker initialization failed:",n),ve("Failed to initialize service worker. Some features may not work offline.")}})}async function pe(){try{let n=await navigator.serviceWorker.register(j.path,{scope:j.scope});console.log("ServiceWorker registered successfully:",n.scope),me(n),fe(n),ye(),ge()}catch(n){throw console.error("ServiceWorker registration failed:",n),n}}function me(n){n.addEventListener("updatefound",()=>{let i=n.installing;i.addEventListener("statechange",()=>{i.state==="installed"&&navigator.serviceWorker.controller&&i.postMessage({type:"SKIP_WAITING"})})})}function fe(n){J(n),setInterval(()=>{J(n)},j.updateCheckInterval)}async function J(n){try{await n.update();let i=await fetch("/_products/product_data.json",{cache:"no-store",headers:{Accept:"application/json"}});if(i.ok){let p=(await i.json()).version,l=localStorage.getItem("productDataVersion");p!==l&&(n.active?.postMessage({type:"INVALIDATE_PRODUCT_CACHE"}),localStorage.setItem("productDataVersion",p),he(null,"New product data available"))}}catch(i){console.warn("Update check failed:",i)}}function ye(){let n=!1;navigator.serviceWorker.addEventListener("controllerchange",()=>{n||(n=!0,window.location.reload())})}function ge(){let n=()=>{let i=document.getElementById("offline-indicator");i&&(i.style.display=navigator.onLine?"none":"block"),navigator.onLine||be("You are currently offline. Some features may be limited.")};window.addEventListener("online",n),window.addEventListener("offline",n),n()}function he(n,i="Una versi\xF3n est\xE1 disponible"){let d=F(i,"Actualizar ahora","Despu\xE9s",()=>{n?n.postMessage({type:"SKIP_WAITING"}):window.location.reload()});H(d)}function ve(n){let i=F(n,"Reload","Dismiss",()=>window.location.reload());H(i)}function be(n){let i=F(n,"Retry","Dismiss",()=>window.location.reload());H(i)}function F(n,i,d,p){let l=document.createElement("div");return l.className="notification-toast",l.setAttribute("role","alert"),l.setAttribute("aria-live","polite"),l.innerHTML=`
