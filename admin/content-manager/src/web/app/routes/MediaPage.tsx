  ...
  return (
    <main role="main" aria-label="Medios">
      ...
      <nav aria-label="Navegación principal" style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
        ...
        <a href="/media" aria-current="page">Medios</a>
      </nav>
      ...
      <div style={{ display: "flex", gap: "0.75rem", marginBottom: "1rem", flexWrap: "wrap" }}>
        {[
          ...
          <button
            ...
            style={{
              ...
              border: filter === key ? "2px solid var(--color-primary)" : "1px solid var(--color-border)",
        ...
        {filter !== "all" && (
          <button onClick={() => setFilter("all")} style={{ padding: "0.5rem 0.75rem", fontSize: "0.85rem" }}>
            Mostrar todos
          </button>
        )}
      </div>
      ...
      {filtered.length > 0 && (
        <table aria-label="Inventario de medios" style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.path} style={{ borderBottom: "1px solid var(--color-border)" }}>
                <td style={{ padding: "0.25rem 0.5rem", maxWidth: "300px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={item.path}>
                  {item.path}
                </td>
...
function StatusBadge({ status }: { status: string }): React.ReactElement {
  ...
  return (
    <span style={{ background: c.bg, color: c.text, padding: "0.1rem 0.4rem", borderRadius: "10px", fontSize: "0.8rem", fontWeight: 500 }}>
      {status}
    </span>
  );
}
