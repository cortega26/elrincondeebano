...
import type { CategoryRecord, NavGroupRecord, Subcategory } from "../../../shared/schemas/category.ts";
...
export function CategoriesPage(): React.ReactElement {
  ...
  const [editingSub, setEditingSub] = useState<{ categoryId: string; subcategory: Subcategory } | null>(null);
  ...
  async function handleCreateSub(categoryId: string, form: Record<string, unknown>): Promise<void> {
    try {
      await client.createSubcategory(categoryId, form as { id: string; title: string; product_key: string; slug: string });
      setAddingSub(null);
  ...
  async function handleUpdateSub(categoryId: string, subId: string, changes: Record<string, unknown>): Promise<void> {
    try {
  ...
  return (
    <main role="main" aria-label="Categorías">
      ...
      <nav aria-label="Navegación principal" style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
        ...
        <a href="/categories" aria-current="page">Categorías</a>
      </nav>
      ...
      <section aria-label="Grupos de navegación" style={{ marginBottom: "1.5rem" }}>
        <h2>
          ...
          <button onClick={() => setAddingGroup(true)} style={{ marginLeft: "0.5rem", fontSize: "0.9rem", padding: "0.1rem 0.5rem" }}>+ Añadir</button>
        </h2>
        {addingGroup && (
          <CategoryForm
            fields={["id", "display_name"]}
            onSave={(form) => { void handleSaveGroup(form); }}
            onCancel={() => setAddingGroup(false)}
          />
        )}
        {groups.length > 0 && (
          <table aria-label="Grupos" style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.9rem" }}>
            <thead>
              <tr>
            ...
            <tbody>
              {groups.map((g) => (
                <tr key={g.id}>
                  <td style={{ padding: "0.25rem" }}><code>{g.id}</code></td>
                  <td style={{ padding: "0.25rem" }}>{g.display_name?.default ?? g.id}</td>
                  <td style={{ padding: "0.25rem", textAlign: "center" }}>{g.active !== false ? "✓" : "✗"}</td>
                  <td style={{ padding: "0.25rem" }}>
                    <button style={{ fontSize: "0.8rem", padding: "0.1rem 0.3rem" }} onClick={() => { void handleDeleteGroup(g.id); }}>Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section aria-label="Categorías">
        <h2>
          Categorías
          <button onClick={() => setAdding(true)} style={{ marginLeft: "0.5rem", fontSize: "0.9rem", padding: "0.1rem 0.5rem" }}>+ Añadir</button>
        </h2>
        {adding && (
          <CategoryForm
                  ...
                  <tr key={cat.id}>
                    <td style={{ padding: "0.25rem" }}>
                      <button
                        ...
                        aria-label={expandedCategories.has(cat.id) ? "Colapsar subcategorías" : "Expandir subcategorías"}
                      >
                        {expandedCategories.has(cat.id) ? "▼" : "▶"}
                      </button>
                    </td>
                    <td style={{ padding: "0.25rem" }}><code>{cat.key}</code></td>
                    ...
                    <td style={{ padding: "0.25rem", textAlign: "center" }}>{cat.active !== false ? "✓" : "✗"}</td>
                    <td style={{ padding: "0.25rem", whiteSpace: "nowrap" }}>
                      <button style={{ fontSize: "0.8rem", padding: "0.1rem 0.3rem" }} onClick={() => setEditing(cat)}>Editar</button>{" "}
                      <button style={{ fontSize: "0.8rem", padding: "0.1rem 0.3rem" }} onClick={() => { void handleDelete(cat.id); }}>Eliminar</button>
                    </td>
                  </tr>
                  {expandedCategories.has(cat.id) && (
                    <tr key={`${cat.id}-sub`}>
                      <td colSpan={8} style={{ padding: "0.5rem 1rem", background: "#fafafa" }}>
                        ...
                        {addingSub === cat.id && (
                          <SubcategoryForm
                            categoryId={cat.id}
                            onSave={(form) => { void handleCreateSub(cat.id, form); }}
                        ...
                        {editingSub && editingSub.categoryId === cat.id && (
                          <SubcategoryForm
                            ...
                            onSave={(form) => { void handleUpdateSub(cat.id, editingSub.subcategory.id, form); }}
                        ...
                        {(cat.subcategories ?? []).length > 0 ? (
                          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" }}>
                            <thead>
                              <tr>
                                <th style={{ textAlign: "left", padding: "0.15rem 0.25rem" }}>ID</th>
                                <th style={{ textAlign: "left", padding: "0.15rem 0.25rem" }}>Title</th>
                                <th style={{ textAlign: "left", padding: "0.15rem 0.25rem" }}>Key</th>
                                <th style={{ textAlign: "left", padding: "0.15rem 0.25rem" }}>Slug</th>
                            ...
                            <tbody>
                              {(cat.subcategories ?? []).map((sub) => (
                                <tr key={sub.id}>
                                  <td style={{ padding: "0.15rem 0.25rem" }}><code>{sub.id}</code></td>
                                  ...
                                  <td style={{ padding: "0.15rem 0.25rem", textAlign: "center" }}>{sub.enabled ? "✓" : "✗"}</td>
                                  <td style={{ padding: "0.15rem 0.25rem", whiteSpace: "nowrap" }}>
                                    <button
                                      style={{ fontSize: "0.7rem", padding: "0.05rem 0.25rem" }}
                                      onClick={() => setEditingSub({ categoryId: cat.id, subcategory: sub })}
                                    >
                                      Editar
                                    </button>{" "}
                                    <button
                                      style={{ fontSize: "0.7rem", padding: "0.05rem 0.25rem" }}
                                      onClick={() => { void handleDeleteSub(cat.id, sub.id); }}
                                    >
                                      Eliminar
                                    </button>
                                  </td>
                          ...
                          <p style={{ fontSize: "0.85rem", color: "#6c757d" }}>No hay subcategorías</p>
                        )}
                      </td>
                    </tr>
                  )}
...
function CategoryForm({
  ...
  function handleSubmit(e: React.FormEvent): void {
    ...
    for (const f of fields) {
      ...
      if (f === "display_name") output.display_name = form.display_name ? { default: form.display_name } : undefined;
  ...
  return (
    <form onSubmit={handleSubmit} style={{ margin: "0.5rem 0", padding: "0.5rem", border: "2px solid var(--color-primary)", borderRadius: "var(--radius)", display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
      {fields.map((f) => (
        <label key={f} style={{ display: "flex", alignItems: "center", gap: "0.25rem", fontSize: "0.85rem" }}>
          {f}:
          {f === "active" ? (
            <select value={form.active} onChange={(e) => setForm((prev) => ({ ...prev, active: e.target.value }))} style={{ padding: "0.15rem" }}>
              <option value="true">Sí</option>
              <option value="false">No</option>
            </select>
          ) : (
            <input value={form[f] ?? ""} onChange={(e) => setForm((prev) => ({ ...prev, [f]: e.target.value }))} style={{ padding: "0.15rem 0.3rem", width: f === "display_name" ? "180px" : "100px" }} />
          )}
        </label>
      ))}
      <button type="submit" style={{ padding: "0.15rem 0.5rem", fontSize: "0.85rem" }}>Guardar</button>
      <button type="button" onClick={onCancel} style={{ padding: "0.15rem 0.5rem", fontSize: "0.85rem" }}>Cancelar</button>
    </form>
  );
}
...
function SubcategoryForm({
  ...
  return (
    <form onSubmit={handleSubmit} style={{ margin: "0.5rem 0", padding: "0.5rem", border: "1px solid var(--color-border)", borderRadius: "var(--radius)", display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center", fontSize: "0.85rem" }}>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        ID:
        <input required value={id} onChange={(e) => setId(e.target.value)} style={{ padding: "0.15rem 0.3rem", width: "100px" }} disabled={!!initial} />
      </label>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        Title:
        <input required value={title} onChange={(e) => setTitle(e.target.value)} style={{ padding: "0.15rem 0.3rem", width: "120px" }} />
      </label>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        Key:
        <input required value={productKey} onChange={(e) => setProductKey(e.target.value)} style={{ padding: "0.15rem 0.3rem", width: "100px" }} />
      </label>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        Slug:
        <input required value={slug} onChange={(e) => setSlug(e.target.value)} style={{ padding: "0.15rem 0.3rem", width: "120px" }} />
      </label>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        Orden:
        <input type="number" value={order} onChange={(e) => setOrder(e.target.value)} style={{ padding: "0.15rem 0.3rem", width: "60px" }} />
      </label>
      <label style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
        Activa:
        <select value={enabled ? "true" : "false"} onChange={(e) => setEnabled(e.target.value === "true")} style={{ padding: "0.15rem" }}>
          <option value="true">Sí</option>
          <option value="false">No</option>
        </select>
      </label>
      <button type="submit" style={{ padding: "0.15rem 0.5rem", fontSize: "0.85rem" }}>{initial ? "Actualizar" : "Crear"}</button>
      <button type="button" onClick={onCancel} style={{ padding: "0.15rem 0.5rem", fontSize: "0.85rem" }}>Cancelar</button>
    </form>
  );
}
