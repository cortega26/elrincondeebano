...
export function ConflictsPage(): React.ReactElement {
  ...
  const [fieldResolutions, setFieldResolutions] = useState<
    Record<string, { resolution: string; manualValue: string }>
  >({});
  ...
  function getFieldResolution(
    conflictId: string,
    field: string
  ): { resolution: string; manualValue: string } {
  ...
  function setFieldResolution(
    conflictId: string,
    field: string,
    value: { resolution: string; manualValue: string }
  ): void {
  ...
  async function handleResolveField(conflictId: string, field: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          (errBody as { error?: { message?: string } }).error?.message ?? `HTTP ${response.status}`
        );
      }
  ...
  return (
    <main role="main" aria-label="Conflictos">
          ...
          {[
            ...
            >
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>
                {data.summary[s.key as keyof ConflictsSummary]}
              </div>
      ...
      {/* Tabs */}
      <nav
        aria-label="Filtros de conflictos"
                  ...
                  return (
                    <tr
                      ...
                      style={{
                        ...
                        background: isResolved ? '#f9fbe7' : 'transparent',
                      }}
                    >
                      ...
                      <td style={{ padding: '0.25rem 0.5rem', textAlign: 'center' }}>
                        {isResolved ? (
                          ...
                          >
                            {fc.resolution}
                            {fc.manual_value !== undefined
                              ? `: ${JSON.stringify(fc.manual_value)}`
                              : ''}
                          </span>
                          ...
                          >
                            <select
                              value={fr.resolution}
                              onChange={(e) =>
                                setFieldResolution(conflict.id, fc.field, {
                                  ...fr,
                            ...
                            <button
                              onClick={() => {
                                void handleResolveField(conflict.id, fc.field);
                              }}
