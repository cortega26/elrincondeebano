...
export function ImportPage(): React.ReactElement {
  ...
  const handlePreview = async (): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setError((err as { error?: { message?: string } }).error?.message ?? `Error ${response.status}`);
        setLoading(false);
        return;
      }
  ...
  const handleResolution = (index: number, resolution: "keep_local" | "use_incoming"): void => {
    setConflicts((prev) =>
      prev.map((c, i) =>
        i === index ? { ...c, resolved: true, resolution } : c,
      ),
    );
  };
  ...
  return (
    <main role="main" aria-label="Importar catálogo">
      ...
      <nav aria-label="Navegación principal" style={{ display: "flex", gap: "1rem", marginBottom: "1rem" }}>
        ...
        <a href="/import" aria-current="page">Importar</a>
      </nav>
      ...
      {conflicts.length > 0 && (
        <div style={{ marginBottom: "1rem" }}>
          <p>
            ...
            <strong>{resolvedCount}</strong> resueltos,{" "}
            <strong>{pendingCount}</strong> pendientes
          </p>

          <table aria-label="Conflictos de importación" style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
            ...
            <tbody>
              {conflicts.map((c, i) => (
                <tr key={`${c.product_id}-${c.field}-${i}`} style={{ borderBottom: "1px solid var(--color-border)" }}>
                  ...
                  <td style={{ padding: "0.25rem 0.5rem" }}>
                    {c.field === "validation" ? (
                      <span style={{ color: "#c62828" }}>
                        Error: {String(c.incoming_value)}
                      </span>
                    ) : (
                      <select
                        value={c.resolution ?? ""}
                        onChange={(e) =>
                          handleResolution(i, e.currentTarget.value as "keep_local" | "use_incoming")
                        }
                      >
