...
export async function storefrontMutRoutes(
  ...
  app.put('/storefront/bundles', async (request, reply) => {
    // Plan 104: no blanket 400 conversion — validation replies are inline;
    // internal errors (repository load, catalog parse) propagate to the
    // central handler as 500 with server-side logging.
  ...
  app.put('/storefront/featured', async (request, reply) => {
    // Plan 104: see the bundles handler — internal errors stay internal.
