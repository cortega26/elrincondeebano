...
def render_admin_ui():
    ...
    if page == "📦 Productos":
        ...
        edit_sku = st.text_input(
            "SKU del producto a editar (dejar vacío para crear nuevo)", key="edit_sku"
        )
        ...
        with st.form("product_form"):
            ...
            image_path = st.text_input("Ruta de imagen",
                                       value=existing.image_path if existing else "")
            stock = st.checkbox("En stock", value=existing.stock if existing else True)
            is_archived = st.checkbox("Archivado",
                                      value=existing.is_archived if existing else False)
    ...
    # ─── Página: Combos ──────────────────────────────────────────
        ...
        if bundles:
            for bundle in bundles:
                with st.expander(f"{bundle.title} ({bundle.bundle_id})"):
                    ...
                    if st.button(f"🗑️ Eliminar {bundle.bundle_id}",
                                 key=f"del_{bundle.bundle_id}"):
        ...
        with st.form("bundle_form"):
            ...
            bundle_price = st.number_input("Precio fijo (0 = suma de productos)",
