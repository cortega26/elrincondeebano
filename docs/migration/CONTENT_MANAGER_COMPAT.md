...
| Offline fallback source                    | `static/offline.html`                | ✅                          | `astro-poc/scripts/sync-data.mjs` copies to `public/pages/offline.html` to avoid drift.                                                            |
...
| Image contract (`image_path`)              | Required                             | ✅                          | Used in card/detail rendering; sync now accepts only `assets/images/**` paths.                                                                     |
| AVIF optional contract (`image_avif_path`) | Optional fallback chain              | ✅                          | `astro-poc/src/components/ProductCard.astro`, `astro-poc/src/components/ProductDetail.astro`; when present, sync also enforces `assets/images/**`. |
...
| Non-canonical image roots       | ✅     | Sync rejects `image_path` / `image_avif_path` outside `assets/images/**`.                             |
