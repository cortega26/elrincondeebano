...
| Product data or category taxonomy                 | `data/`, `admin/content-manager/` (canónico — `npm run admin:dev`); `admin/product_manager/` es el **fallback** durante la ventana de transición (plan 069) | `npm run admin:validate && npm run build && npm run guardrails:assets`              | `docs/architecture/CATEGORY_REGISTRY.md`, `admin/content-manager/README.md` |
