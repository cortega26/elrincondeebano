...
- **`Admin Tools CI`** (`.github/workflows/admin.yml`) — cambios en `admin/**`. Python 3.14: `product_manager` con ruff/bandit/pytest desde `requirements-dev.txt` (fallback, hasta el retiro del plan 069); Content Manager TS con lint, typecheck, vitest, coverage, build, E2E Playwright (smoke + import + changes + media + storefront), drills de rollback, y audit, cerrado por el certification report (`--ci`: gatea solo los checks automatizados; las filas de paridad y los drills las firma el mantenedor, ver `CUTOVER.md`).
