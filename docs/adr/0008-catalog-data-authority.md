...
and root `data/`), and `astro-poc/src/data/products.json` / `categories.json` are stale
artifacts that no process currently writes.

## Field inventory

### Product fields across all models

| Field                  | models.py |  data_store.py  | productStore.js | data-schemas.ts  |
| ---------------------- | :-------: | :-------------: | :-------------: | :--------------: |
| `name`                 | required  |    required     |    required     | required (min 1) |
| `description`          | optional  |    optional     |    optional     |     optional     |
| `price`                | required  |    optional     |    required     |     optional     |
| `discount`             | optional  |    optional     |    optional     |     optional     |
| `stock`                | optional  |    optional     |    optional     |     optional     |
| `category`             | optional  |    required     |    optional     | required (min 1) |
| `image_path`           | optional  |    optional     |    optional     |     optional     |
| `image_avif_path`      | optional  |    optional     |    optional     |     optional     |
| `order` / `sort_order` | optional  |    optional     |    optional     |     optional     |
| `is_archived`          | optional  |    optional     |    optional     |     optional     |
| `brand`                |     —     |    optional     |    optional     |     optional     |
| `thumbnail_path`       |     —     |    optional     |    optional     |     optional     |
| `image_variants`       |     —     |        —        |        —        |     optional     |
| `thumbnail_variants`   |     —     |        —        |        —        |     optional     |
| `sku`                  |     —     | required UNIQUE |    optional     |     optional     |
| `id`                   |     —     |        —        |    optional     |     optional     |
| `slug`                 |     —     |        —        |    optional     |        —         |
| `rev`                  |    int    |        —        |       int       |   optional int   |
| `field_last_modified`  |   dict    |        —        |      dict       |        —         |
...
| `astro-poc/src/data/products.json`              | _nothing writes this_                         | **stale artifact**              |
| `astro-poc/src/data/categories.json`            | _nothing writes this_                         | **stale artifact**              |
...
4. **`astro-poc/src/data/products.json` and `astro-poc/src/data/categories.json` are
   stale migration artifacts.** No process writes them. The Astro build reads from
   `astro-poc/src/data/products.json` at `catalog.ts:109` — this is a legacy read path
   that must be redirected to `data/product_data.json` as part of the migration.
