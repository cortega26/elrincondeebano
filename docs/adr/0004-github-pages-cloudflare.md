...
- **Cloudflare may challenge GitHub-hosted CI runners (histórico).** En julio de 2026 se migraron los workflows `live-contract-monitor.yml` y `post-deploy-canary.yml` a runners GitHub-hosted `ubuntu-24.04`. Los desafíos Cloudflare que motivaron el runner self-hosted en 2025 ya no se reproducen con la configuración actual de Cloudflare. La decisión original de usar self-hosted queda archivada como contexto histórico; el runtime activo es GitHub-hosted.
